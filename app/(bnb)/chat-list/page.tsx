...
const ChatListPage = () => {
  ...
  return <div className={"w-full h-fit bg-white px-2"}>
    <p className={"p-5 mt-5 font-bold text-center text-xl"}>스타일리스트</p>
    <div className={"flex flex-col space-y-2 px-5"}>
      <div className={"w-full p-5 rounded-2xl bg-gray-100 text-sm space-y-2 flex flex-col items-center mb-2.5"}>
        <Image src={"/chat-list/envelope-open.svg"} alt={""} width={20} height={20} />
        <p className={"font-semibold"}>당신이 모르는 아름다움을 찾기 위해 매일 실험하고 있어요.</p>
        <p className={"text-center mb-5"}>Valyou는 사용자들의 피드백을 통해 매일 개선해요.</p>
      ...
      <button className={"w-full border border-gray-200 rounded-2xl flex flex-col justify-center items-center mb-10"} onClick={() => {
        window.open("https://open.kakao.com/o/sPpbZKZh");
      }}>
        <div className={"w-full h-44 relative border-b border-gray-200 bg-[#EFEFEF] rounded-t-2xl"}>
...
const PersonaRoom = ({ persona, index }: PersonaRoom) => {
  ...
  return <motion.button
    ...
    className={"w-full h-fit shadow-xl flex flex-col space-y-4 rounded-2xl"} onClick={() => router.push(`/chat/${persona.name}`)}>
    <div className={"w-full aspect-square relative"}>
      { persona.avatar_image ? <Image src={persona.avatar_image} alt={persona.display_name} draggable={false} fill className={"rounded-2xl object-cover"} /> : <div className={"w-24 h-24 bg-gray-100 rounded-2xl flex items-center justify-center"}>
